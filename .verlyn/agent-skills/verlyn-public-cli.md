...
When MCP is configured, use the mixed posture from
`Documentation/guides/VERLYN_CLI_MCP_TRANSITION.md`: CLI-first, MCP-optional,
CLI fallback. MCP may be used only for explicitly supported API-backed workflow
actions after OAuth succeeds; keep CLI for bootstrap, local checkout state,
governance, trust checks, review-runner orchestration, and hosted delivery/deploy.
...
- treat review-generated remediation work items as complete structural work
  orders, not narrow patch prompts. For split-file, complexity, branching,
  broad exception handling, security-boundary, or test-matrix findings, work
  the named domain/subdomain and every affected surface before rerunning
  independent review. Finish or explicitly disposition the whole review-created
  batch first.
...
Do not restore repo-local support script dependencies for generated governance
PR gates.

Verlyn source checkouts may use `verlyn-source-pr` and
`verlyn-source-tester-lane` for source-specific regression lanes. Generated
governance for other repositories must stay on the standalone profiles.
...
`Documentation/guides/VERLYN_CLI_MCP_TRANSITION.md`, this skill file, and the
Codex adapter at `.verlyn/.codex/skills/verlyn-public-cli/SKILL.md`.
